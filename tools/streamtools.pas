...
begin
  ...
  LBase64String := base64.EncodeStringBase64(AString);
  if Length(LBase64String) > 0 then
    Result.WriteBuffer(LBase64String[1], Length(LBase64String));
  Result.Position := 0;
end;
...
var
  LString: string;
begin
  SetLength(LString, AStream.Size);
  AStream.Position := 0;
  if AStream.Size > 0 then
    AStream.ReadBuffer(LString[1], AStream.Size);
  Result := base64.EncodeStringBase64(LString);
end;
